...
export default function PolicyDeletion({ props }) {
  ...
  useEffect(() => {
    if (!values.fetched && motorPolicyData) {
      ...
      if (!additionalFetched) {
        ...
        let additionalPrems = data.additionalPremiums.length - 1;
        let newAddBen = [];
        let newAddPrem = [];
  ...
  const onChange = useCallback(
    (event, { name, value }) => {
      if (name === "minimumPremiumAmount" && value > 0) {
        value = -value;
      }
  ...
  const [policyDeletion, { loading }] = useCallback(
    useMutation(POLICY_DELETION, {
      update(_, result) {
        context.endorsePolicyDeletion(result.data);
        let pol = result.data.createPolicyDeletion.policyDeletion;

        context.endorsePolicyDeletion(
          result.data.createPolicyDeletion.policyDeletion
        );
        history.push({
  ...
  useEffect(() => {
    if (values.insuranceCompany && insuranceCos) {
      let comp = insuranceCos.find((x) => x.id === values.insuranceCompany.id);
          ...
          <div className="content-wrapper">
            <Header as="h3">
              ...
              <Header.Content>
                ...
                {">"} Policy Deletion
                <Header.Subheader>
                  Fill this form to make a deletion to a policy
                </Header.Subheader>
              </Header.Content>
            </Header>
          </div>
        ...
        <Grid.Column>
          ...
          >
            ...
            <Form.Group widths="equal">
              ...
              <Form.Field required error={errors.errorPaths.includes("corporateClient")}>
