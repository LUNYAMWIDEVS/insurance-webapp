...
from .models import PolicyRenewal, AdditionalBenefit
from .object_types import PolicyRenewalObjectType, PolicyRenewalInput
...
class CreatePolicyRenewal(graphene.Mutation):
    """
    Create Policy Renewal
    """

    class Arguments:
        input = PolicyRenewalInput(required=True)

    # items that the mutation will return
    policy_renewal = graphene.Field(PolicyRenewalObjectType)
    ...
    def mutate(self, info, input):
        ...
        policy_renewal = PolicyRenewal(**input)
        policy_renewal.save()
        for benefit in additional_benefits:
            benefit.pop("amount", "")
            add_ben = AdditionalBenefit(**benefit)
            add_ben.save()
            policy_renewal.additional_benefits.add(add_ben)
        for premium in additional_premiums:
            ...
            add_prem.save()
            policy_renewal.additional_premiums.add(add_prem)
        ...
        for policy_detail_set in policy_detail_sets:
            ...
            policy_renewal.policy_detail_set.add(detail_)

        return CreatePolicyRenewal(
            status="Success",
            policy_renewal=policy_renewal,
            message=SUCCESS_ACTION.format("Policy renewed"),
        )
...
class Mutation(graphene.ObjectType):
    ...
    create_policy_renewal = CreatePolicyRenewal.Field()
