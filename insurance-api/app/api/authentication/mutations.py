...
from ..agency.object_types import AgencyInput
from ..agency.validators.validate_agency import AgencyValidations
...
class CreateAdmin(graphene.Mutation):
    ...
    class Arguments:
        ...
        agency = AgencyInput(required=True)
    ...
    def mutate(self, info, **kwargs):
        ...
        agency_validator = AgencyValidations()
        admin_data = admin_validator.validate_user_registration_data(
            kwargs.get("admin", ''))
        agency_data = agency_validator.validate_agency_registration_data(
            kwargs.get("agency", ''))

        new_agency = Agency(**agency_data)
        new_agency.save()
        admin_data['agency'] = new_agency
        new_user = User.objects.create_superuser(**admin_data)
        assign_role(new_user, 'admin')

        agency_link = '{}agencies/activate/{}'.format(
            f"{info.context.get_raw_uri()}".replace(".109", ".109:8087"),
            generate_jwt(new_agency.agency_email, new_agency.id))
        agency_data = {
            'username': new_agency.name,
            'link': agency_link,
            'insurance_agency': new_agency.name,
            'message_body': AGENCY_MAIL_BODY_MSG,
            "body": ACCOUNT_ACTIVATION_MSG
        }
        mail_subject = "{} account activation".format(new_agency.name)
        message = "Verify your agency account"
        template_email('verify_account.html', new_agency.agency_email, mail_subject,
                       message, **agency_data)
        admin_link = '{}users/activate/{}'.format(
            f"{info.context.get_raw_uri()}".replace(".109", ".109:8087"),
            generate_jwt(new_user.email, new_user.agency.id))
        admin_data = {
            'username': new_user.username,
            'link': admin_link,
            'insurance_agency': new_agency.name,
            'message_body': ADMIN_MAIL_BODY_MSG.format(new_agency.name),
            "body": ACCOUNT_ACTIVATION_MSG
        }
        mail_subject = "{} admin account activation".format(new_agency.name)
...
class CreateUser(graphene.Mutation):
    ...
    def mutate(self, info, **kwargs):
        ...
        user_data = {
            'username': new_user.username,
            'link': user_link,
            'insurance_agency': new_user.agency.name,
            'message_body': USER_MAIL_BODY_MSG.format(new_user.agency.name),
            'body': ACCOUNT_ACTIVATION_MSG
        }
        mail_subject = "Account activation for {}".format(new_user.agency.name)
        message = "Verify your account"
        template_email('verify_account.html', new_user.email, mail_subject,
                       message, **user_data)
...
class PasswordReset(graphene.Mutation):
    ...
    def mutate(root, info, **kwargs):
        ...
        user_link = '{}reset-password/{}'.format(
            ...
            generate_jwt(user.email, user.agency.id, password_reset=True,
        ...
        mail_subject = "Password reset for {}".format(user.agency.name)
