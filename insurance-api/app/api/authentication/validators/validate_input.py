...
class UserValidations:
    ...
    def validate_user_update_data(self, data, user):
        ...
        for item in data:
            ...
            if item == 'phone_number':
                self.validate_phone_number(data.get(item), user.agency)
    ...
    def validate_email_already_exist(cls, email, agency):
        ...
        '''
        try:
            email_existing = User.objects.all_with_deleted().get(email=email,
                                                                 agency=agency)
        except ObjectDoesNotExist:
            email_existing = None

        if email_existing and email_existing.is_active:
            raise GraphQLError(error_dict['already_exist'].format('email'))
    ...
    def validate_phone_number(cls, phone_number, agency):
        ...
        if User.objects.all_with_deleted().filter(phone_number=phone_number,
