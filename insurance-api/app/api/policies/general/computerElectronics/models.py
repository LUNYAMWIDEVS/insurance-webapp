...
class Property(BaseModel):
    """
    Computer Electronics Policy model
    """
    ...
    history = HistoricalRecords(table_name="history_computer_electronics_property",
...
class ComputerElectronic(BasePolicyModel):
    """
    Computer Electronics Policy model
    """
    ...
    computer_electronic_properties = models.ManyToManyField(Property)
    agency = models.ForeignKey(Agency, on_delete=models.CASCADE,
                               related_name="%(app_label)s_%(class)s_related", null=False)

    history = HistoricalRecords(table_name="history_computer_electronics",
                                history_change_reason_field=ArrayField(models.CharField(
                                    max_length=250, blank=True,
                                    unique=False, null=True), blank=True,
                                    unique=False, null=True),
