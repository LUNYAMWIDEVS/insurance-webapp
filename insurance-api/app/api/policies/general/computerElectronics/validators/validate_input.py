...
from app.api.helpers.constants import (FIRE_DETAILS_REQUIRED_FIELD,
                                       COMPUTER_ELECTRONIC_REQUIRED_FIELD)
...
class ComputerElectronicValidations:
    '''Validations for the burglary policy'''

    def validate_computer_electronic_data(self, kwargs, agency):
        '''
        Runs all the burglary policy data validations in one function
        Args:
        ...
        input_data['computer_electronic_properties'] = kwargs.get('computer_electronic_properties', None)
        ...
        check_missing_fields(input_data['computer_electronic_properties'],
                             FIRE_DETAILS_REQUIRED_FIELD)
        _ = [check_empty_fields(item) for item in input_data['computer_electronic_properties']]

        self.validate_policy_no(input_data['policy_no'], agency)
        self.validate_choice_fields(input_data['transaction_type'],
                                    dict(ComputerElectronic.TransactionType.choices),
                                    'transaction type')
        self.validate_choice_fields(input_data['premium_type'],
                                    dict(ComputerElectronic.PremiumType.choices),
                                    'premium type')
    ...
    def validate_package_details_update(self, kwargs):
        '''
        Runs all the burglary policy details data update validations in one function
        Args:
            kwargs (dict): request data
        Returns:
            input_data (dict): validated data
    ...
    def validate_computer_electronic_data_update(self, kwargs, agency):
        '''
        Runs all the burglary policy data update validations in one function
        Args:
            kwargs (dict): request data
        Returns:
            input_data (dict): validated data
