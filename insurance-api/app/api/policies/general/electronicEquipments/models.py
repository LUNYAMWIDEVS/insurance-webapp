...
class Property(BaseModel):
    """
    ElectronicEquipment Policy model
    """
    ...
    history = HistoricalRecords(table_name="history_electronics_property",
...
class ElectronicEquipment(BasePolicyModel):
    """
    ElectronicEquipment Policy model
    """
    ...
    electronic_equipment_properties = models.ManyToManyField(Property)
    agency = models.ForeignKey(Agency, on_delete=models.CASCADE,
                               related_name="%(app_label)s_%(class)s_related", null=False)

    history = HistoricalRecords(table_name="history_electronics_policy",
                                history_change_reason_field=ArrayField(models.CharField(
                                    max_length=250, blank=True,
                                    unique=False, null=True), blank=True,
                                    unique=False, null=True),
    ...
    def __str__(self):
        """
        Returns a string representation of this `ElectronicEquipmentPolicy`.

        This string is used when a `ElectronicEquipmentPolicy` is printed in the console.
        """
