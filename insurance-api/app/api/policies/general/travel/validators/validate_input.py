...
from app.api.helpers.constants import (TRAVEL_DETAILS_REQUIRED_FIELD,
                                       TRAVEL_REQUIRED_FIELD)
...
class TravelValidations:
    '''Validations for the travel'''

    def validate_travel_data(self, kwargs, agency):
        '''
        Runs all the travel data validations in one function
        Args:
        ...
        '''
        check_missing_fields(kwargs, TRAVEL_REQUIRED_FIELD)
        ...
        input_data['travel_details'] = kwargs.get('travel_details', None)
        input_data['individual_client'] = kwargs.get('individual_client', None)
        check_empty_fields(data=input_data)
        check_missing_fields(input_data['travel_details'],
                             TRAVEL_DETAILS_REQUIRED_FIELD)
        check_empty_fields(data=input_data['travel_details'])
        ...
        self.validate_choice_fields(input_data['transaction_type'],
                                    dict(Travel.TransactionType.choices),
                                    'transaction type')
        self.validate_choice_fields(input_data['premium_type'],
                                    dict(Travel.PremiumType.choices),
                                    'premium type')
    ...
    def validate_travel_details_update(self, kwargs):
        '''
        Runs all the travel details data update validations in one function
        Args:
            kwargs (dict): request data
        Returns:
            input_data (dict): validated data
        '''
        input_data = {}
        input_data['option'] = kwargs.get('option', None)
        input_data['date_of_travel'] = kwargs.get('date_of_travel', None)
        input_data['countries_of_travel'] = kwargs.get('countries_of_travel', None)
        input_data['modes_of_travel'] = kwargs.get('modes_of_travel', None)
        input_data['reasons_of_travel'] = kwargs.get('reasons_of_travel', None)
        input_data['next_of_kin'] = kwargs.get('next_of_kin', None)

        input_data = {k: v for k, v in input_data.items() if v}
        return input_data

    def validate_travel_data_update(self, kwargs, agency):
        '''
        Runs all the travel data update validations in one function
        Args:
            kwargs (dict): request data
        Returns:
            input_data (dict): validated data
