...
class Property(BaseModel):
    ...
    history = HistoricalRecords(table_name="history_contractors_risks",
...
class ContractorsRisk(BasePolicyModel):
    """
    Contractpr  Policy model
    """
    ...
    contractor_properties = models.ManyToManyField(Property)
    agency = models.ForeignKey(Agency, on_delete=models.CASCADE,
                            related_name="%(app_label)s_%(class)s_related", null=False)

    history = HistoricalRecords(table_name="history_contractors_policy",
                                history_change_reason_field=ArrayField(models.CharField(
                                    max_length=250, blank=True,
                                    unique=False, null=True), blank=True,
                                    unique=False, null=True),
    ...
    def __str__(self):
        """
        Returns a string representation of this `Contractpr Risks`.

        This string is used when a `Contractpr Risks` is printed in the console.
        """
