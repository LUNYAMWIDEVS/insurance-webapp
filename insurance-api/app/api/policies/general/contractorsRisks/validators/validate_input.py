...
from ..models import ContractorsRisk
...
class ContractorsRiskValidations:
    '''Validations for the burglary policy'''

    def validate_contractors_risk_data(self, kwargs, agency):
        '''
        ...
        input_data['contractor_properties'] = kwargs.get('contractor_properties', None)
        ...
        check_missing_fields(input_data['contractor_properties'],
                             FIRE_DETAILS_REQUIRED_FIELD)
        _ = [check_empty_fields(item) for item in input_data['contractor_properties']]

        self.validate_policy_no(input_data['policy_no'], agency)
        self.validate_choice_fields(input_data['transaction_type'],
                                    dict(ContractorsRisk.TransactionType.choices),
                                    'transaction type')
        self.validate_choice_fields(input_data['premium_type'],
                                    dict(ContractorsRisk.PremiumType.choices),
                                    'premium type')
    ...
    def validate_contractors_risk_data_update(self, kwargs, agency):
        '''
    ...
    def validate_debit_note_no(self, debit_note_no, agency):
        ...
        '''
        if ContractorsRisk.objects.all_with_deleted().filter(
