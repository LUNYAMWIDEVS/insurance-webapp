...
from ..models import GoodsTransit
...
class GoodsTransitValidations:
    '''Validations for the burglary policy'''

    def validate_goods_transit_data(self, kwargs, agency):
        '''
        ...
        input_data['goods_transit_properties'] = kwargs.get('goods_transit_properties', None)
        ...
        check_missing_fields(input_data['goods_transit_properties'],
                             FIRE_DETAILS_REQUIRED_FIELD)
        _ = [check_empty_fields(item) for item in input_data['goods_transit_properties']]

        self.validate_policy_no(input_data['policy_no'], agency)
        self.validate_choice_fields(input_data['transaction_type'],
                                    dict(GoodsTransit.TransactionType.choices),
                                    'transaction type')
        self.validate_choice_fields(input_data['premium_type'],
                                    dict(GoodsTransit.PremiumType.choices),
                                    'premium type')
    ...
    def validate_goods_transit_data_update(self, kwargs, agency):
        '''
    ...
    def validate_debit_note_no(self, debit_note_no, agency):
        ...
        '''
        if GoodsTransit.objects.all_with_deleted().filter(
