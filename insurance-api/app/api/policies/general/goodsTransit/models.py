...
class Property(BaseModel):
    ...
    history = HistoricalRecords(table_name="history_goodsTransit_properties",
...
class GoodsTransit(BasePolicyModel):
    """
    Burglary Policy model
    """
    ...
    goods_transit_properties = models.ManyToManyField(Property)
    agency = models.ForeignKey(Agency, on_delete=models.CASCADE,
                               related_name="%(app_label)s_%(class)s_related", null=False)

    history = HistoricalRecords(table_name="history_goodsTransit_policy",
                                history_change_reason_field=ArrayField(models.CharField(
                                    max_length=250, blank=True,
                                    unique=False, null=True), blank=True,
                                    unique=False, null=True),
    ...
    def __str__(self):
        """
        Returns a string representation of this `GoodsTransit`.

        This string is used when a `GoodsTransit` is printed in the console.
        """
