...
from .object_types import (GoodsTransitPaginatedType, GoodsTransitType,
                           GoodsTransitPropertyType)
from .helpers.burglary_policy_helpers import get_default_options


class Query(ObjectType):
    goods_transit_options = GenericScalar()
    goods_transit = graphene.Field(
        GoodsTransitType, id=graphene.String())
    goods_transit_property = graphene.Field(
        GoodsTransitPropertyType, id=graphene.String())
    goods_transits = graphene.Field(GoodsTransitPaginatedType,
                                       page=graphene.Int(),
    ...
    @token_required
    @login_required
    def resolve_goods_transit_options(self, info, **kwargs):
        return get_default_options()

    @token_required
    @login_required
    def resolve_goods_transit_property(self, info, **kwargs):
        id = kwargs.get('id', None)
        return validate_object_id(id, Property, "GoodsTransit Policy property")

    @token_required
    @login_required
    def resolve_goods_transit(self, info, **kwargs):
        id = kwargs.get('id', None)
        return validate_object_id(id, GoodsTransit, "GoodsTransit Policy",
                                  info.context.user.agency)

    @token_required
    @login_required
    def resolve_goods_transits(self, info, search=None, **kwargs):
        ...
        if search:
            ...
            clients = GoodsTransit.objects.filter(
            ...
            clients = GoodsTransit.objects.filter(
                agency=info.context.user.agency).all().order_by(
                    'individual_client__first_name')
        return pagination_helper(clients, page, limit, GoodsTransitPaginatedType)
