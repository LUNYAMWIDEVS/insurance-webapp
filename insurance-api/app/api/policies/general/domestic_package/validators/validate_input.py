...
from app.api.helpers.constants import (PACKAGE_DETAILS_REQUIRED_FIELD,
                                       DOMESTIC_PACKAGE_REQUIRED_FIELD)
...
class DomesticPackageValidations:
    '''Validations for the domestic package'''

    def validate_domestic_package_data(self, kwargs, agency):
        '''
        Runs all the domestic package data validations in one function
        Args:
        ...
        '''
        check_missing_fields(kwargs, DOMESTIC_PACKAGE_REQUIRED_FIELD)
        ...
        input_data['package_details'] = kwargs.get('package_details', None)
        ...
        check_missing_fields(input_data['package_details'],
                             PACKAGE_DETAILS_REQUIRED_FIELD)
        check_empty_fields(data=input_data['package_details'])
        self.validate_policy_no(input_data['policy_no'], agency)
        self.validate_choice_fields(input_data['transaction_type'],
                                    dict(DomesticPackage.TransactionType.choices),
                                    'transaction type')
        self.validate_choice_fields(input_data['premium_type'],
                                    dict(DomesticPackage.PremiumType.choices),
                                    'premium type')
    ...
    def validate_package_details_update(self, kwargs):
        '''
        Runs all the domestic package details data update validations in one function
        Args:
            kwargs (dict): request data
        Returns:
            input_data (dict): validated data
        '''
        input_data = {}
        input_data['buildings'] = kwargs.get('buildings', None)
        input_data['contents'] = kwargs.get('contents', None)
        input_data['all_risks'] = kwargs.get('all_risks', None)
        input_data['work_man_injury'] = kwargs.get('work_man_injury', None)
        input_data['occupiers_liability'] = kwargs.get('occupiers_liability', None)
        input_data['owner_liability'] = kwargs.get('owner_liability', None)

        input_data = {k: v for k, v in input_data.items() if v}
        return input_data

    def validate_domestic_package_data_update(self, kwargs, agency):
        '''
        Runs all the domestic package data update validations in one function
        Args:
            kwargs (dict): request data
        Returns:
            input_data (dict): validated data
    ...
    def validate_policy_no(self, policy_no, agency):
        ...
        if DomesticPackage.objects.all_with_deleted().filter(
            ...
            raise GraphQLError(error_dict['already_exist'].format(
                'domestic package policy number'))
