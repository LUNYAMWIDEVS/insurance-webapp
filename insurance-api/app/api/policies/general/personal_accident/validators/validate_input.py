...
from app.api.helpers.constants import (PERSONAL_ACC_REQUIRED_FIELD)
...
class PersonalAccidentValidations:
    '''Validations for the personal accident'''

    def validate_personal_accident_data(self, kwargs, agency):
        '''
        Runs all the personal accident data validations in one function
        Args:
        ...
        '''
        check_missing_fields(kwargs, PERSONAL_ACC_REQUIRED_FIELD)
        ...
        input_data['benefit_limits'] = kwargs.get('benefit_limits', None)
        ...
        self.validate_choice_fields(input_data['transaction_type'],
                                    dict(PersonalAccident.TransactionType.choices),
                                    'transaction type')
        self.validate_choice_fields(input_data['benefit_limits'],
                                    dict(PersonalAccident.BenefitsType.choices),
                                    'benefit/limit type')
        self.validate_choice_fields(input_data['premium_type'],
                                    dict(PersonalAccident.PremiumType.choices),
                                    'premium type')
    ...
    def validate_personal_accident_data_update(self, kwargs, agency):
        '''
        Runs all the personal accident data update validations in one function
        Args:
            kwargs (dict): request data
        Returns:
            input_data (dict): validated data
        ...
        VAL_MAP = {
            ...
            "benefit_limits": {
                "method": self.validate_choice_fields,
                'params': [input_data.get('benefit_limits', ''),
                           dict(PersonalAccident.BenefitsType.choices),
                           'benefit/limit type']},
            "insurance_company": {
