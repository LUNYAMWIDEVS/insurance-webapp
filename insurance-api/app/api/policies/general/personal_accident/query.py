...
from .helpers.personal_accident_helpers import get_default_options
from .models import PersonalAccident
from .object_types import PersonalAccidentPaginatedType, PersonalAccidentType


class Query(ObjectType):
    personal_accident_options = GenericScalar()
    personal_accident = graphene.Field(
        PersonalAccidentType, id=graphene.String())
    personal_accidents = graphene.Field(PersonalAccidentPaginatedType,
                                        page=graphene.Int(),
    ...
    @token_required
    @login_required
    def resolve_personal_accident_options(self, info, **kwargs):
        return get_default_options()

    @token_required
    @login_required
    def resolve_personal_accident(self, info, **kwargs):
        id = kwargs.get('id', None)
        return validate_object_id(id, PersonalAccident, "Personal accident",
                                  info.context.user.agency)

    @token_required
    @login_required
    def resolve_personal_accidents(self, info, search=None, **kwargs):
        ...
        if search:
            filter = (
                ...
                Q(benefit_limits__icontains=search) |
                Q(renewal_date__icontains=search))
            clients = PersonalAccident.objects.filter(
            ...
            clients = PersonalAccident.objects.filter(
                agency=info.context.user.agency).all().order_by(
                    'individual_client__first_name')
        return pagination_helper(clients, page, limit, PersonalAccidentPaginatedType)
