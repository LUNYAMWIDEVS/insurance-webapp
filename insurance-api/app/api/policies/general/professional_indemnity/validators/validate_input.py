...
from app.api.helpers.constants import (PROFESSIONAL_INDEMNITY_REQUIRED_FIELD)
...
class ProfessionalIndemnityValidations:
    '''Validations for the professional indemnity'''

    def validate_prof_ind_policy_data(self, kwargs, agency):
        '''
        Runs all the professional indemnity data validations in one function
        Args:
        ...
        '''
        check_missing_fields(kwargs, PROFESSIONAL_INDEMNITY_REQUIRED_FIELD)

        input_data = {}
        input_data['debit_note_no'] = kwargs.get('debit_note_no', None)
        input_data['sum_insured'] = kwargs.get('sum_insured', None)
        ...
        self.validate_choice_fields(input_data['specialty_class'],
                                    dict(ProfessionalIndemnity.SpecialtyClass.choices),
                                    'specialty class')
        self.validate_choice_fields(input_data['transaction_type'],
                                    dict(ProfessionalIndemnity.TransactionType.choices),
                                    'transaction type')
        self.validate_choice_fields(input_data['premium_type'],
                                    dict(ProfessionalIndemnity.PremiumType.choices),
                                    'premium type')
    ...
    def validate_prof_ind_policy_data_update(self, kwargs, agency):
        '''
        Runs all the professional indemnity data update validations in one function
        Args:
            kwargs (dict): request data
        Returns:
            input_data (dict): validated data
