...
class PlateGlassValidations:
    '''Validations for the plate_glass policy'''

    def validate_plate_glass_data(self, kwargs, agency):
        '''
        Runs all the plate_glass policy data validations in one function
        Args:
        ...
        input_data['plate_glass_properties'] = kwargs.get('plate_glass_properties', None)
        ...
        check_missing_fields(input_data['plate_glass_properties'],
                             FIRE_DETAILS_REQUIRED_FIELD)
        _ = [check_empty_fields(item) for item in input_data['plate_glass_properties']]

        self.validate_policy_no(input_data['policy_no'], agency)
        self.validate_choice_fields(input_data['transaction_type'],
                                    dict(PlateGlass.TransactionType.choices),
                                    'transaction type')
        self.validate_choice_fields(input_data['premium_type'],
                                    dict(PlateGlass.PremiumType.choices),
                                    'premium type')
    ...
    def validate_package_details_update(self, kwargs):
        '''
        Runs all the plate_glass policy details data update validations in one function
        Args:
            kwargs (dict): request data
        Returns:
            input_data (dict): validated data
    ...
    def validate_plate_glass_data_update(self, kwargs, agency):
        '''
        Runs all the plate_glass policy data update validations in one function
        Args:
            kwargs (dict): request data
        Returns:
            input_data (dict): validated data
