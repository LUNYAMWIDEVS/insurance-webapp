...
class IndustrialAllRiskValidations:
    '''Validations for the industrial_all_risks policy'''

    def validate_industrial_all_risks_data(self, kwargs, agency):
        '''
        Runs all the industrial_all_risks policy data validations in one function
        Args:
        ...
        input_data['industrial_all_risks_properties'] = kwargs.get('industrial_all_risks_properties', None)
        ...
        check_missing_fields(input_data['industrial_all_risks_properties'],
                             FIRE_DETAILS_REQUIRED_FIELD)
        _ = [check_empty_fields(item) for item in input_data['industrial_all_risks_properties']]

        self.validate_policy_no(input_data['policy_no'], agency)
        self.validate_choice_fields(input_data['transaction_type'],
                                    dict(IndustrialAllRisk.TransactionType.choices),
                                    'transaction type')
        self.validate_choice_fields(input_data['premium_type'],
                                    dict(IndustrialAllRisk.PremiumType.choices),
                                    'premium type')
    ...
    def validate_package_details_update(self, kwargs):
        '''
        Runs all the industrial_all_risks policy details data update validations in one function
        Args:
            kwargs (dict): request data
        Returns:
            input_data (dict): validated data
    ...
    def validate_industrial_all_risks_data_update(self, kwargs, agency):
        '''
        Runs all the industrial_all_risks policy data update validations in one function
        Args:
            kwargs (dict): request data
        Returns:
            input_data (dict): validated data
    ...
    def validate_policy_no(self, policy_no, agency):
        ...
        if IndustrialAllRisk.objects.all_with_deleted().filter(
            ...
            raise GraphQLError(error_dict['already_exist'].format(
                'industrial_all_risks policynumber'))
