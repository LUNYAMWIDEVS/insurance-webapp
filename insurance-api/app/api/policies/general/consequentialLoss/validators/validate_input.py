...
from app.api.helpers.constants import (FIRE_DETAILS_REQUIRED_FIELD,
                                       CONSEQUENTIAL_LOSS_REQUIRED_FIELD)
...
class ConsequentialLossValidations:
    '''Validations for the consequential_loss policy'''

    def validate_consequential_loss_policy_data(self, kwargs, agency):
        '''
        Runs all the consequential_loss policy data validations in one function
        Args:
        ...
        '''
        check_missing_fields(kwargs, CONSEQUENTIAL_LOSS_REQUIRED_FIELD)
        ...
        input_data['consequential_loss_properties'] = kwargs.get('consequential_loss_properties', None)
        ...
        check_missing_fields(input_data['consequential_loss_properties'],
                             FIRE_DETAILS_REQUIRED_FIELD)
        _ = [check_empty_fields(item) for item in input_data['consequential_loss_properties']]

        self.validate_policy_no(input_data['policy_no'], agency)
        self.validate_choice_fields(input_data['transaction_type'],
                                    dict(ConsequentialLoss.TransactionType.choices),
                                    'transaction type')
        self.validate_choice_fields(input_data['premium_type'],
                                    dict(ConsequentialLoss.PremiumType.choices),
                                    'premium type')
    ...
    def validate_package_details_update(self, kwargs):
        '''
        Runs all the consequential_loss policy details data update validations in one function
        Args:
            kwargs (dict): request data
        Returns:
            input_data (dict): validated data
    ...
    def validate_consequential_loss_policy_data_update(self, kwargs, agency):
        '''
        Runs all the consequential_loss policy data update validations in one function
        Args:
            kwargs (dict): request data
        Returns:
            input_data (dict): validated data
    ...
    def validate_policy_no(self, policy_no, agency):
        ...
        if ConsequentialLoss.objects.all_with_deleted().filter(
            ...
            raise GraphQLError(error_dict['already_exist'].format(
                'consequential_loss policynumber'))
