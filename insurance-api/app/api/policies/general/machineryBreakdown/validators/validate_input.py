...
class MachineryBreakdownValidations:
    '''Validations for the machinery_breakdown policy'''

    def validate_machinery_breakdown_data(self, kwargs, agency):
        '''
        Runs all the machinery_breakdown policy data validations in one function
        Args:
        ...
        input_data['machinery_breakdown_properties'] = kwargs.get('machinery_breakdown_properties', None)
        ...
        check_missing_fields(input_data['machinery_breakdown_properties'],
                             FIRE_DETAILS_REQUIRED_FIELD)
        _ = [check_empty_fields(item) for item in input_data['machinery_breakdown_properties']]

        self.validate_policy_no(input_data['policy_no'], agency)
        self.validate_choice_fields(input_data['transaction_type'],
                                    dict(MachineryBreakdown.TransactionType.choices),
                                    'transaction type')
        self.validate_choice_fields(input_data['premium_type'],
                                    dict(MachineryBreakdown.PremiumType.choices),
                                    'premium type')
    ...
    def validate_package_details_update(self, kwargs):
        '''
        Runs all the machinery_breakdown policy details data update validations in one function
        Args:
            kwargs (dict): request data
        Returns:
            input_data (dict): validated data
    ...
    def validate_machinery_breakdown_data_update(self, kwargs, agency):
        '''
        Runs all the machinery_breakdown policy data update validations in one function
        Args:
            kwargs (dict): request data
        Returns:
            input_data (dict): validated data
    ...
    def validate_policy_no(self, policy_no, agency):
        ...
        if MachineryBreakdown.objects.all_with_deleted().filter(
            ...
            raise GraphQLError(error_dict['already_exist'].format(
                'machinery_breakdown policynumber'))
