...
from app.api.helpers.constants import (WIBA_POLICY_REQUIRED_FIELD)
...
class WibaPolicyValidations:
    '''Validations for the wiba policy'''

    def validate_wiba_policy_data(self, kwargs, agency):
        '''
        Runs all the wiba policy data validations in one function
        Args:
        ...
        '''
        check_missing_fields(kwargs, WIBA_POLICY_REQUIRED_FIELD)
        ...
        input_data['no_of_staff'] = kwargs.get('no_of_staff', None)
        input_data['estimate_annual_earning'] = kwargs.get(
            'estimate_annual_earning', None)
        ...
        self.validate_choice_fields(input_data['transaction_type'],
                                    dict(WibaPolicy.TransactionType.choices),
                                    'transaction type')
        self.validate_choice_fields(input_data['premium_type'],
                                    dict(WibaPolicy.PremiumType.choices),
                                    'premium type')
    ...
    def validate_wiba_policy_data_update(self, kwargs, agency):
        '''
        Runs all the wiba policy data update validations in one function
        Args:
            kwargs (dict): request data
        Returns:
            input_data (dict): validated data
