...
class BurglaryPolicyValidations:
    '''Validations for the burglary policy'''

    def validate_burglary_policy_data(self, kwargs, agency):
        '''
        ...
        input_data['burglary_properties'] = kwargs.get('burglary_properties', None)
        ...
        check_missing_fields(input_data['burglary_properties'],
                             FIRE_DETAILS_REQUIRED_FIELD)
        _ = [check_empty_fields(item) for item in input_data['burglary_properties']]

        self.validate_policy_no(input_data['policy_no'], agency)
        self.validate_choice_fields(input_data['transaction_type'],
                                    dict(BurglaryPolicy.TransactionType.choices),
                                    'transaction type')
        self.validate_choice_fields(input_data['premium_type'],
                                    dict(BurglaryPolicy.PremiumType.choices),
                                    'premium type')
    ...
    def validate_burglary_policy_data_update(self, kwargs, agency):
        '''
    ...
    def validate_debit_note_no(self, debit_note_no, agency):
        ...
        '''
        if BurglaryPolicy.objects.all_with_deleted().filter(
