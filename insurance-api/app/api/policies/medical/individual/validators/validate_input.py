...
from app.api.helpers.constants import (INDIVIDUAL_MEDICAL_REQUIRED_FIELD,
                                       MEDICAL_INS_REQUIRED_FIELD)
...
class IndividualMedicalInsValidations:
    '''Validations for the individual medical'''

    def validate_individual_medical_policy_data(self, kwargs, agency):
        '''
        Runs all the individual medical data validations in one function
        Args:
        ...
        '''
        check_missing_fields(kwargs, INDIVIDUAL_MEDICAL_REQUIRED_FIELD)
        ...
        input_data['medical_insurance'] = kwargs.get('medical_insurance', None)
        ...
        check_missing_fields(input_data['medical_insurance'], MEDICAL_INS_REQUIRED_FIELD)
        ...
        self.validate_choice_fields(input_data['transaction_type'],
                                    dict(IndividualMedicalIns.TransactionType.choices),
                                    'transaction type')
        self.validate_choice_fields(input_data['premium_type'],
                                    dict(IndividualMedicalIns.PremiumType.choices),
                                    'premium type')
    ...
    def validate_individual_medical_policy_data_update(self, kwargs, agency):
        '''
        Runs all the individual medical data update validations in one function
        Args:
            kwargs (dict): request data
        Returns:
            input_data (dict): validated data
    ...
    def validate_policy_no(self, policy_no, agency):
        ...
        if IndividualMedicalIns.objects.all_with_deleted().filter(
            ...
            raise GraphQLError(error_dict['already_exist'].format(
                'individual medical policy number'))
